...
static inline uint8_t psa_cipher_to_sgi_alg(const psa_algorithm_t alg, const mcuxClCipher_ModeDescriptor_t **mode)
{
                                      ...
                                      size_t *output_length)
{
    ...
    if ((alg == PSA_ALG_CBC_NO_PADDING || alg == PSA_ALG_CBC_PKCS7 || alg == PSA_ALG_ECB_NO_PADDING) &&
    ...
    /* uint32_t ivLength:                       */ iv_length,
    /* mcuxCl_InputBuffer_t pIn:                 */ (mcuxCl_Buffer_t) (input + iv_length),
    /* uint32_t inLength:                       */ input_length - iv_length,
    /* mcuxCl_Buffer_t pOut:                     */ output,
    /* uint32_t * const outLength:              */ &expected_op_length)
  );
