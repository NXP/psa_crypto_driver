...
psa_status_t sgi_hash_setup(mcux_sgi_hash_operation_t *operation, psa_algorithm_t alg)
{
    ...
    MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(result, token,
                                     mcuxClHash_init(
                                         ...
                                         /* mcuxClHash_Context_t context:   */ (mcuxClHash_Context_t)&operation->ctx,
                                         ...
                                         size_t input_length)
{
    ...
    MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(
        result, token, mcuxClHash_process(session, (mcuxClHash_Context_t)&operation->ctx, input, input_length));
                                         ...
                                         size_t *hash_length)
{
    ...
    *hash_length = p_hash_data->ctx.algo->hashSize;
    ...
    MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(result, token,
                                     mcuxClHash_finish(session, &p_hash_data->ctx, hash, &hashOutputSize));
...
psa_status_t sgi_hash_abort(mcux_sgi_hash_operation_t *operation)
{
    ...
    MCUX_CSSL_FP_FUNCTION_CALL_VOID_BEGIN(
        token, mcuxClMemory_clear((uint8_t *)operation->ctx, MCUXCLHASH_CONTEXT_SIZE_SHA2_512_IN_WORDS,
                                          ...
                                          size_t *hash_length)
{
    ...
    MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(result, token,
                                     mcuxClHash_compute(session, hashAlgo, input, input_length, hash, &hashOutputSize));
